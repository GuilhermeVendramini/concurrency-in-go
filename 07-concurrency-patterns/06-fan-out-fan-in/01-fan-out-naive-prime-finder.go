...
/*
	<1> Here we take in our standard done channel to allow our goroutines to be torn
	down, and then a variadic slice of interface{} channels to fan-in.

	<2> On this line we create a sync.WaitGroup so that we can wait until all channels
	have been drained.

	<3> Here we create a function, multiplex , which, when passed a channel, will read
	from the channel, and pass the value read onto the multiplexedStream channel.
